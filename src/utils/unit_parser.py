...
class UnitParser:
    ...
    def parse_torque(torque: float, system_unit: int) -> tuple[float, str]:
        ...
        if system_unit == 0:
            if _torque < 10000:
                ...
                return [float(f'{_torque/1000:.2f}'), 'kN·m']
        else:
            _metric_torque = UnitConverter.nm_to_gfm(_torque)
            if _metric_torque < 10000:
                return [float(f'{_metric_torque:.2f}'), 'gf·m']
            else:
                return [float(f'{_metric_torque/1000:.2f}'), 'kgf·m']
    ...
    def parse_thrust(thrust: float, system_unit: int) -> tuple[float, str]:
        ...
        if system_unit == 0:
            ...
            if _metric_thrust < 10000:
                return [float(f'{_metric_thrust:.2f}'), 'gf']
            else:
                return [float(f'{_metric_thrust/1000:.2f}'), 'kgf']
