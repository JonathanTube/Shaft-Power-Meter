...
class PermissionCheck(ft.AlertDialog):
    def __init__(self,  on_confirm: Callable, user_role: int, closable: bool = True):
        super().__init__()
    ...
    def build(self):
        ...
        self.actions = [
            ...
            ft.TextButton(s.get("lang.button.cancel"), visible=self.closable, on_click=lambda e: self.page.close(self))
        ]
    ...
    def __on_confirm(self, e):
        ...
        user = User.select().where(User.user_role <= self.user_role and User.user_name == user_name).first()
