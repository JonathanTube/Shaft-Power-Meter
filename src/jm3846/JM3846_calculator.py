...
class JM3846Calculator:
    ...
    def calculate_microstrain(self, mv_per_v: float) -> float:
        ...
        if self.config.sensitivity_factor_k == 0:
            return 0
        result = mv_per_v / self.config.sensitivity_factor_k * (10**3)
    ...
    def calculate_torque(self, microstrain: float) -> float:
        ...
        numerator = math.pi * microstrain * \
            self.config.elastic_modulus_E * (D**4 - d**4)
    ...
    def calculate_thrust(self, mv_per_v: float) -> float:
        ...
        numerator = 2 * mv_per_v * math.pi * self.config.elastic_modulus_E * diameter_diff
